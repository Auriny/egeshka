...
# За один ход игрок может добавить в кучу один или три камня либо увеличить количество камней в куче в четыре раза. Для того, чтобы делать ходы,
...
#
# Задание 21.
# Для игры, описанной в задании 19, найдите минимальное значение S, при котором одновременно выполняются дваусловия:
...
print('19)', [s for s in range(1, 110 + 1) if f(s, 2)])
print('20)', [s for s in range(1, 110 + 1) if not f(s, 1) and f(s, 3)])
print('21)', [s for s in range(1, 110 + 1) if not f(s, 2) and f(s, 4)])
